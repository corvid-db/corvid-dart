...
# for this host (Windows), then normalize the artifacts into deps\current for
# the Dart FFI loader (and ffigen). macOS/Linux: fetch.sh.
#
...
# ---- normalize into deps\current (what the loader + ffigen point at) ------
# A stable directory name keeps the loader's search path platform-
# independent: deps\current\corvid.h + corvid.dll. The Dart loader opens
# deps\current\corvid.dll by absolute path, so PATH does not need to change;
# the CORVID_LIBRARY env var overrides the location entirely.
...
Write-Host "fetch: deps\current ready (corvid.h, corvid.dll) -- pin $CorvidVersion"
