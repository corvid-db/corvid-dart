...
# this host (macOS/Linux), then normalize the artifacts into deps/current for
# the Dart FFI loader (and ffigen). Windows: fetch.ps1.
#
...
# Required: corvid.h always; plus the cdylib for this platform. The
# parentheses matter — without them && binds tighter than || and the
# corvid.h check goes inert on Linux (SC2235 acknowledged: no subshell
# side effects here).
...
# independent: deps/current/corvid.h + libcorvid.dylib|libcorvid.so.
